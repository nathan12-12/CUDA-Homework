...
void timer();
void setup();
__global__ void leapFrog(float3 *, float3 *, float3 *, float *, float, float, int);
__global__ void thenMove(float3 *, float3 *, float3 *, float *, float, float, float, int);
void nBody();
int main(int, char**);
...
{	
    ...
    nBody();
    cudaDeviceSynchronize(); // Ensure that the GPU finished before timing stops
	cudaErrorCheck(__FILE__, __LINE__);
    gettimeofday(&end, NULL);
    drawPicture();
    	
	computeTime = elaspedTime(start, end);
	printf("The compute time was %ld microseconds.\n", computeTime);
	printf("The compute time was %f seconds.\n\n", float(computeTime)/1000000.0f); // In seconds
}
...
void setup()
{
    ...
    BlockSize.x = (N > 1024) ? BLOCK_SIZE : N; // Since the limit is 1024 threads per block, but if N < 1024 just use N
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void leapFrog(float3 *p, float3 *f, float *m, float g, float h, int n) // Reduce the input, some of them called later in move function
{
	float dx, dy, dz,d,d2;
	float force_mag;
	...
	if(i < n)
	{
		...
		for(int j = 0; j < n; j++)
		{
			if(i != j) // Don't calculate yourself now
			{
				dx = p[j].x-p[i].x;
				dy = p[j].y-p[i].y;
				dz = p[j].z-p[i].z;
...
__global__ void thenMove(float3 *p, float3 *v, float3 *f, float *m, float damp, float dt, float t, int n) // Damp, dt, v, and t are now here
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;
	
	if(i < n)
	{
		if(t == 0.0f) // The leap frog is here now but whatever
		{
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		// Two functions needed since blocks can't sync
		leapFrog<<<GridSize,BlockSize>>>(PGPU, FGPU, MGPU, G, H, N); // Calculate forces
		cudaErrorCheck(__FILE__, __LINE__);
		thenMove<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, Damp, dt, t, N); // Then move
		cudaErrorCheck(__FILE__, __LINE__);
