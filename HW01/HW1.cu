...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{ 
		c[id] = a[id] + b[id]; // Every element a is the index and b is 2 times the index, so c will be 3 times the index.
	}
}
...
bool check(float *c, int n, float tolerence)
{
	...
	for(id = 0; id < n; id++)
	{ 
		myAnswer += c[id]; // Add every single element in the vector together by using the pointer
	}
	
	trueAnswer = 3.0*(m*(m+1))/2.0;
	
	percentError = abs((myAnswer - trueAnswer)/trueAnswer)*100.0;
	printf("\n\n My answer was %lf and the true answer is %lf", myAnswer, trueAnswer);
	printf("\n This is a percent error of %lf %%", percentError);
...
int main()
{
	...
	for(int i = 0; i < 5; i++) // Basically every element is 3 times the index
	{
		printf("\nValue of C[%d]: %f", i, C_CPU[i]);
	}
	...
	if(check(C_CPU, N, Tolerance) == false) // Tolerance = 0.01
	{
	...
	else
	{
		...
		printf("\n The time it took was %ld microseconds (accounting for the additional lines of code)", elaspedTime(start, end));
	}
