...
int N, DrawFlag;
float3 *P, *V, *F;
float3 *PGPU, *VGPU, *FGPU; // GPU Pointers
float *M; 
float *MGPU; // GPU Mass Pointers
...
void timer();
void setup();
// Need 1 GPU Function to calculate Forces and move afterwards
__global__ void doTheThing(float3 *, float3 *, float3 *, float *, float, float, float, float, float, int);
void nBody();
int main(int, char**);
...
void gpuSetup()
{
	...
	// Randomly setting these bodies in the global sphere and setting the initial velosity, inotial force, and mass.
	for(int i = 0; i < N; i++)
	{
		test = 0;
		while(test == 0)
...
__global__ void doTheThing(float3 *p, float3 *v, float3 *f, float *m, float g, float h, float damp, float dt, float t, int n)
{
	float dx, dy, dz,d,d2;
	float force_mag;
	
	int i = threadIdx.x + blockDim.x * blockIdx.x;
	if(i > n) return; // Then its done
	f[i].x = 0.0f;
	f[i].y = 0.0f;
	f[i].z = 0.0f;
	...
	__syncthreads(); // Before moving the bodies, synchronize first
	
	// At first, calculate half velocity
	if(t == 0.0f) {
		v[i].x += ((f[i].x-damp*v[i].x)/m[i])*dt/2.0f;
		v[i].y += ((f[i].y-damp*v[i].y)/m[i])*dt/2.0f;
		v[i].z += ((f[i].z-damp*v[i].z)/m[i])*dt/2.0f;
	}
	// Then, continue to step over by 1
	else {
		v[i].x += ((f[i].x-damp*v[i].x)/m[i])*dt;
		v[i].y += ((f[i].y-damp*v[i].y)/m[i])*dt;
		v[i].z += ((f[i].z-damp*v[i].z)/m[i])*dt;
	}
	// For positions always step by 1
	p[i].x += v[i].x*dt;
	p[i].y += v[i].y*dt;
	p[i].z += v[i].z*dt;
	__syncthreads(); // Synchronize after each movement
}
/*
void setup()
{
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		doTheThing<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, Damp, dt, t, N);
		cudaErrorCheck(__FILE__, __LINE__);
		if(drawCount == DRAW_RATE) 
		{
			if(DrawFlag) 	
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("nBody With 1 Block");
