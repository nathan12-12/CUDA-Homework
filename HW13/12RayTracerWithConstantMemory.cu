...
__constant__ sphereStruct SpheresGPU[NUMSPHERES]; // Put the sphere info into constant memory on the GPU.
...
__global__ void makeSphersBitMap(float *pixels) // This makes all threads read from SphereGPU (constant memory).
{
	...
	{
		hitValue = hit(pixelx, pixely, &dimingValue, SpheresGPU[i]); // This makes all threads read from SphereGPU (constant memory).
