...
#include <sys/time.h>
#include <stdio.h>
#include <math.h> // For trig and sqrt functions

// Defines
#define N 11503 // Length of the vector
// With the complicated addition, the GPU was faster, even at n = 5000
...
void setUpDevices()
{
	...
	GridSize.x = 64; //(N - 1)/BlockSize.x + 1; // This gives us the correct number of blocks.
	GridSize.y = 1;
	GridSize.z = 1;
}
...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{ 
		c[id] = sqrt(cos(a[id]) * cos(a[id]) + a[id] * a[id] + sin(a[id]) * sin(a[id]) - 1.0)
             + sqrt(cos(b[id]) * cos(b[id]) + b[id] * b[id] + sin(b[id]) * sin(b[id]) - 1.0);
	} // Complicated way to do a + b
}
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
    ...
    #pragma unroll // Tells the compiler to unroll the loop for faster execution, in this case it was slightly faster
	/* 
	Because by allowing more instructions to be executed in parallel and potentially reducing unnessary branching with if or switch statements.
	Where the condition being checked either remains constant throughout the loop's execution or could be evaluated less frequently.
	The #pragma unroll directives can only be used on for loops. They cannot be applied to do while and while loops.
	The loop structure must meet the following conditions:
    1. There must be only one loop counter variable, one increment point for that variable, and one termination variable. These cannot be altered at any point in the loop nest.
    2. Loops cannot have multiple entry and exit points. The loop termination must be the only means to exit the loop.
    3. Dependencies in the loop must not be "backwards-looking". For example, a statement such as A[i][j] = A[i -1][j + 1] + 4 must not appear within the loop.
	*/
    for (int i = id; i < n; i += stride)
    {
        c[i] = sqrt(cos(a[i]) * cos(a[i]) + a[i] * a[i] + sin(a[i]) * sin(a[i]) - 1.0)
             + sqrt(cos(b[i]) * cos(b[i]) + b[i] * b[i] + sin(b[i]) * sin(b[i]) - 1.0);
    }
}
