...
// nvcc -O3 --use_fast_math -maxrregcount=64 -arch=sm_75 V_nBodySpeedChallenge.cu -o temp -lglut -lm -lGLU -lGL -Xptxas="-dlcm=ca,-O3" \
...
// *__restrict__ means "promise me you won't go anywhere while I'm not checking"
__global__ void getForces(float3 *__restrict__ p, float3 *__restrict__ f, float  *__restrict__ m, float g, float h, int n) // velocity, v, is not needed
{
    __shared__ float3 sharedPosition[BLOCK_SIZE];
    __shared__ float sharedMass[BLOCK_SIZE];

    int i = threadIdx.x + blockDim.x * blockIdx.x;
    float3 myPosition, myForce = {0.0f, 0.0f, 0.0f}; // Initialize positions and forces to 0
    float myMass;
    ...
    if (i < n) { // ldcg uses L2 cache might be slower but good for large N, but ldg uses L1 which is faster, honestly I tried they're about the same speed
		...
		myMass = __ldg(&m[i]); // Load the i-th mass
	}
    
    // Pre-compute constant terms (done once per thread, not per interaction)
    float g_mass = g * myMass;
    float h_mass = h * myMass;
	...
	UNROLL for (int tile = 0; tile < gridDim.x; tile++) { // Loop over tiles
        ...
        sharedPosition[threadIdx.x] = (idx < n) ? p[idx] : make_float3(0.0f, 0.0f, 0.0f); // Reduce conditional divergence and initialize unused threads to zero
        sharedMass[threadIdx.x] = (idx < n) ? m[idx] : 0.0f; // Same here
        __syncthreads();
        ...
        if (i < n) {
            UNROLL for (int j = 0; j < blockDim.x; j++) {
                ...
                if (globalJ < n && i != globalJ) {
					...
					// On my testing fmaf is more stable than __fmaf_rz
...
__global__ void moveBodies(float3 *__restrict__ p, float3 *__restrict__ v, float3 *__restrict__ f, float  *__restrict__ m, float damp, float dt, float t, int n)
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;
	float halfDt = dt * 0.5f; // Cannot bit shift floats
	float mass = m[i];
    float inv_mass = __frcp_rn(mass); // Screw precision!!! Hahahaha, but this __frcp_rn for inverse Single Precision floating-point numbers

	if(i < n)
	{
		if(t == 0.0f)
		{
			v[i].x += ((f[i].x - damp * v[i].x) * inv_mass) * halfDt;
			v[i].y += ((f[i].y - damp * v[i].y) * inv_mass) * halfDt;
			v[i].z += ((f[i].z - damp * v[i].z) * inv_mass) * halfDt;
		}
		else
		{
			v[i].x += ((f[i].x - damp * v[i].x) * inv_mass)*dt;
			v[i].y += ((f[i].y - damp * v[i].y) * inv_mass)*dt;
			v[i].z += ((f[i].z - damp * v[i].z) * inv_mass)*dt;
		}
...
void nBody() {
    ...
    while (t < RUN_TIME) {
        ...
        getForces<<<GridSize, BlockSize, 0, computeStream>>>(PGPU, FGPU, MGPU, G, H, N);
        //cudaErrorCheck(__FILE__, __LINE__);
        moveBodies<<<GridSize, BlockSize, 0, computeStream>>>(PGPU, VGPU, FGPU, MGPU, Damp, dt, t, N);
        //cudaErrorCheck(__FILE__, __LINE__);

        // Perform data transfer for visualization in transfer stream
        if (drawCount == DRAW_RATE && DrawFlag) {
            cudaMemcpyAsync(P, PGPU, N * sizeof(float3), cudaMemcpyDeviceToHost, transferStream);
