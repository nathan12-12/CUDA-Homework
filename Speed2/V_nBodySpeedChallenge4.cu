...
void timer();
void setup();
__global__ void getForces(float4 *, float4 *, float4 *, float, float, int);
__global__ void moveBodies(float4 *, float4 *, float4 *, float4 *, float, float, float, int);
void nBody();
int main(int, char**);
...
void setup()
{
	...
	UNROLL for(int i = 0; i < N; i++)
	{
        ...
        // Mass stored in .x, padding zeros
        M[i] = make_float4(1.0f, 0.0f, 0.0f, 0.0f);
	}
...
__global__ void getForces(float4 *__restrict__ p, float4 *__restrict__ f, float4  *__restrict__ m, float g, float h, int n) // velocity, v, is not needed
{
	...
	UNROLL for (int tile = 0; tile < gridDim.x; tile++) { // Loop over tiles
        ...
        sharedMass[threadIdx.x] = (idx < n) ? m[idx].x : 0.0f; // Same here
        __syncthreads();
...
__global__ void moveBodies(float4 *__restrict__ p, float4 *__restrict__ v, float4 *__restrict__ f, float4  *__restrict__ m, float damp, float dt, float t, int n)
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;\
